...
contract Swap  {
    ...
    string [] public tokens = ["Tether USD" , "BNB" , "USD Coin" , "stETH" , "TRON" , "Matic Token" ,
    "SHIBA INU" , "Uniswap"];
    ...
    constructor(){
        for(uint i =0;i<tokens.length;i++){
            CustomSwapToken token = new CustomSwapToken(tokensi],tokens[i]) ;
    ...
    function getTokenAddress( string memory tokenName) public view returns (uint256) {
        ...
        return addres(tokenInstanceMap[tokenName]);
    }

    function _transactionHistory(string memory tokenName, string memory etherToken, uint256 inputValue, uint256 outputValue) internal returns () {
        
        _historyIndex++;
        uint256 _historyId = _historyIndex;
        History storage history = history[_historyId];
